...
void NotificationHandler<SSL>::register_routes(uWS::TemplatedApp<SSL>& app) {
  ...
  app.post("/api/notifications/:id/read", [this](auto* res, auto* req) {
    ...
    std::string body;
    res->onData([res, this, user_id, notification_id, body = std::move(body)](
  ...
  app.post("/api/notifications/read-by-channel/:channelId", [this](auto* res, auto* req) {
    ...
    res->onAborted([]() {});
    std::string body;
    res->onData([res, this, user_id, channel_id, body = std::move(body)](
                  std::string_view chunk, bool last) mutable {
      body.append(chunk);
      if (!last) return;
  ...
  app.post("/api/notifications/read-all", [this](auto* res, auto* req) {
    ...
    std::string body;
    res->onData(
      [res, this, user_id, body = std::move(body)](std::string_view chunk, bool last) mutable {
        body.append(chunk);
        if (!last) return;
