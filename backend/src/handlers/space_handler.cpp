...
void SpaceHandler<SSL>::register_routes(uWS::TemplatedApp<SSL>& app) {
  ...
  app.put("/api/spaces/:id/members/:userId", [this](auto* res, auto* req) {
    ...
    res->onData([this,
      ...
      try {
        ...
        if (
          new_rank < target_rank && target_rank >= actor_space_rank && user_id != target_user_id) {
          res->writeStatus("403")
            ->writeHeader("Content-Type", "application/json")
            ->writeHeader("Access-Control-Allow-Origin", "*")
  ...
  app.post("/api/spaces/:id/archive", [this](auto* res, auto* req) {
    ...
    json notify = {{"type", "space_updated"}, {"space", {{"id", space_id}, {"is_archived", true}}}};
  ...
  app.post("/api/spaces/:id/avatar", [this](auto* res, auto* req) {
    ...
    res->onData([this, res, body, max_size, user_id, space_id, content_type](
                  std::string_view data, bool last) mutable {
      body->append(data);
