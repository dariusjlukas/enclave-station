...
json build_registration_challenge_extra(
...
json build_pki_challenge_extra(
  const std::string& type, const std::optional<std::string>& public_key) {
...
json build_device_passkey_challenge_extra(
  const std::string& user_id, const std::string& device_token) {
  return {{"type", "device_passkey"}, {"user_id", user_id}, {"device_token", device_token}};
}

json build_passkey_registration_options(
  const Config& config,
  ...
  json options = {
    ...
    {"user", {{"id", user_handle}, {"name", username}, {"displayName", display_name}}},
    {"challenge", challenge},
    {"pubKeyCredParams",
     json::array({{{"type", "public-key"}, {"alg", -7}}, {{"type", "public-key"}, {"alg", -257}}})},
...
json build_exclude_credentials(const std::vector<Database::WebAuthnCredential>& credentials) {
  ...
  for (const auto& credential : credentials) {
    json descriptor = {{"type", "public-key"}, {"id", credential.credential_id}};
...
json build_join_request_status_response(
