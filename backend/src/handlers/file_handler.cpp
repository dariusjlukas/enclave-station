...
void FileHandler<SSL>::register_routes(uWS::TemplatedApp<SSL>& app) {
  ...
  app.post("/api/channels/:id/upload", [this](auto* res, auto* req) {
    ...
    res->onData([this, res, body, aborted, channel_id, token, filename, content_type, message_text](
                  std::string_view data, bool last) mutable {
      body->append(data);
      if (!last) return;

      pool_->submit(
        [this, res, body, aborted, channel_id, token, filename, content_type, message_text]() {
          if (*aborted) return;
  ...
  app.post("/api/channels/:id/upload/init", [this](auto* res, auto* req) {
    ...
    res->onData([this, res, body, aborted, channel_id, token](std::string_view data, bool last) {
      body->append(data);
      if (!last) return;

      pool_->submit([this, res, body, aborted, channel_id, token]() {
        if (*aborted) return;
    ...
    res->onData([this, res, body, aborted, token, upload_id, index, expected_hash](
      ...
      pool_->submit([this, res, body, aborted, token, upload_id, index, expected_hash]() {
        ...
        if (!err.empty()) {
          ...
          loop_->defer(
            [res, aborted, status = std::move(status), err_json = std::move(err_json)]() {
              if (*aborted) return;
  ...
  app.post("/api/channels/:id/upload/:uploadId/complete", [this](auto* res, auto* req) {
    ...
    res->onData([this, res, aborted, channel_id, upload_id, token](std::string_view, bool last) {
      if (!last) return;

      pool_->submit([this, res, aborted, channel_id, upload_id, token]() {
        if (*aborted) return;
  ...
  app.get("/api/files/:id", [this](auto* res, auto* req) {
    ...
    pool_->submit([this, res, aborted, file_id, token]() {
      if (*aborted) return;
