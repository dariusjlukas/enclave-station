...
void FileHandler<SSL>::register_routes(uWS::TemplatedApp<SSL>& app) {
  ...
  post_csrf(app, "/api/channels/:id/upload/init", [this](auto* res, auto* req) {
    ...
    res->onData([this, res, body, aborted, scope, channel_id, token, origin](
                  std::string_view data, bool last) {
      body->append(data);
      if (!last) return;

      pool_->submit([this, res, body, aborted, scope, channel_id, token, origin]() {
        if (*aborted) return;
  ...
  post_csrf(app, "/api/channels/:id/upload/:uploadId/complete", [this](auto* res, auto* req) {
    ...
    res->onData([this, res, aborted, scope, channel_id, upload_id, token, origin](
                  std::string_view, bool last) {
      if (!last) return;

      pool_->submit([this, res, aborted, scope, channel_id, upload_id, token, origin]() {
        if (*aborted) return;
  ...
  app.get("/api/files/:id", [this](auto* res, auto* req) {
    ...
    pool_->submit([this, res, aborted, scope, file_id, token, origin]() {
      if (*aborted) return;
