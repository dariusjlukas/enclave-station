...
json build_pki_challenge_extra(
  const std::string& type, const std::optional<std::string>& public_key = std::nullopt);
...
json build_token_user_response(
  const std::string& token, const json& user_json, const json& extra = json::object());
