...
json build_settings_response(const Snapshot& snapshot) {
    return {
        ...
        {"default_space_storage_limit",
         parse_i64_setting_or(snapshot.default_space_storage_limit, 0)},
        {"personal_spaces_enabled",
         parse_bool_setting_or(snapshot.personal_spaces_enabled, false)},
...
std::map<std::string, std::string> collect_settings_updates(const json& settings,
    ...
    if (settings.contains("personal_spaces_total_storage_limit")) {
        ...
        if (value < 0) throw std::runtime_error("Personal spaces total storage limit must be non-negative");
