...
static json page_to_json(const WikiPage& p) {
    return {
        ...
        {"icon", p.icon}, {"cover_image_file_id", p.cover_image_file_id},
        {"position", p.position}, {"is_deleted", p.is_deleted},
...
void WikiHandler<SSL>::register_routes(uWS::TemplatedApp<SSL>& app) {
        ...
        res->onData([this, res, user_id = std::move(user_id_copy),
            ...
            try {
                ...
                if (db.wiki_page_slug_exists(space_id, parent_id, slug)) {
                    int suffix = 2;
                    while (db.wiki_page_slug_exists(space_id, parent_id, slug + "-" + std::to_string(suffix))) {
                        suffix++;
                    }
                    slug = slug + "-" + std::to_string(suffix);
                }

                auto page = db.create_wiki_page(space_id, parent_id, title, slug,
                                                  is_folder, content, content_text, icon,
                                                  position, user_id);
    ...
    app.put("/api/spaces/:id/wiki/pages/:pageId", [this](auto* res, auto* req) {
        ...
        res->onData([this, res, user_id = std::move(user_id_copy),
                     ...
                     page_id = std::move(page_id), body = std::move(body)](
            std::string_view data, bool last) mutable {
            body.append(data);
            if (!last) return;
            if (user_id.empty()) return;
            ...
            try {
                ...
                std::string cover_image_file_id = j.value("cover_image_file_id", existing->cover_image_file_id);
                ...
                if (title != existing->title) {
                    ...
                    if (db.wiki_page_slug_exists(space_id, existing->parent_id, slug, page_id)) {
                        int suffix = 2;
                        while (db.wiki_page_slug_exists(space_id, existing->parent_id,
                                                         slug + "-" + std::to_string(suffix), page_id)) {
                            suffix++;
                        }
                        slug = slug + "-" + std::to_string(suffix);
                    }
                ...
                // Update page
                auto page = db.update_wiki_page(page_id, title, slug, content,
                                                 content_text, icon, cover_image_file_id, user_id);
        ...
        res->onData([this, res, user_id = std::move(user_id_copy),
            ...
            try {
                ...
                for (const auto& pos : positions) {
                    page_positions.emplace_back(pos.at("id").get<std::string>(),
                                                 pos.at("position").get<int>());
                }
    ...
    // Revert to version
    app.post("/api/spaces/:id/wiki/pages/:pageId/versions/:versionId/revert", [this](auto* res, auto* req) {
        auto user_id_copy = get_user_id(res, req);
        std::string space_id(req->getParameter(0));
        std::string page_id(req->getParameter(1));
        std::string version_id(req->getParameter(2));
        ...
        res->onData([this, res, user_id = std::move(user_id_copy),
                     ...
                     version_id = std::move(version_id), body = std::move(body)](
            std::string_view data, bool last) mutable {
            body.append(data);
            if (!last) return;
            if (user_id.empty()) return;
            ...
            try {
                ...
                db.create_wiki_page_version(page_id, page->title,
                                             page->content, page->content_text, user_id);

                // Update page with the version's content
                auto updated = db.update_wiki_page(page_id, version->title, page->slug,
                                                    version->content, version->content_text,
                                                    page->icon, page->cover_image_file_id, user_id);
    ...
    app.get("/api/spaces/:id/wiki/pages/:pageId/permissions", [this](auto* res, auto* req) {
        ...
        json resp = {{"permissions", arr},
                     {"my_permission", get_page_access_level(space_id, page_id, user_id)}};
        res->writeHeader("Content-Type", "application/json")
            ->writeHeader("Access-Control-Allow-Origin", "*")
            ->end(resp.dump());
    });
    ...
    app.post("/api/spaces/:id/wiki/upload/:uploadId/complete", [this](auto* res, auto* req) {
        ...
        res->onData([this, res, space_id, upload_id, user_id](std::string_view, bool last) {
            ...
            try {
                ...
                auto file = db.create_space_file(space_id, "", unique_name,
...
template <bool SSL>
bool WikiHandler<SSL>::check_space_access(uWS::HttpResponse<SSL>* res,
...
template <bool SSL>
std::string WikiHandler<SSL>::get_access_level(const std::string& space_id,
                                                       const std::string& user_id) {
    auto user = db.find_user_by_id(user_id);
...
template <bool SSL>
std::string WikiHandler<SSL>::get_page_access_level(const std::string& space_id,
...
template <bool SSL>
bool WikiHandler<SSL>::require_permission(uWS::HttpResponse<SSL>* res,
                                                 const std::string& space_id,
                                                 const std::string& user_id,
...
template <bool SSL>
bool WikiHandler<SSL>::require_page_permission(uWS::HttpResponse<SSL>* res,
                                                        const std::string& space_id,
                                                        const std::string& page_id,
                                                        const std::string& user_id,
                                                        const std::string& required_level) {
