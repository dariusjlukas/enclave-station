...
void AdminHandler<SSL>::register_routes(uWS::TemplatedApp<SSL>& app) {
  ...
  app.post("/api/admin/server-icon", [this](auto* res, auto* req) {
    ...
    res->onData([this, res, body, max_size, user_id = std::move(user_id_copy)](
                  std::string_view data, bool last) mutable {
      body->append(data);
  ...
  app.put("/api/admin/users/:userId/role", [this](auto* res, auto* req) {
    ...
    res->onData([this,
                 res,
                 actor_id = std::move(actor_id_copy),
...
void AdminHandler<SSL>::save_settings(
  uWS::HttpResponse<SSL>* res, const std::string& body, bool mark_setup) {
  try {
    ...
    if (
      updates.count("personal_spaces_enabled") || updates.count("personal_spaces_files_enabled") ||
...
void AdminHandler<SSL>::handle_approve(
  uWS::HttpResponse<SSL>* res, const std::string& request_id, const std::string& admin_id) {
  try {
