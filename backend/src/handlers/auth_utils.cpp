...
MfaDecision build_mfa_decision(
  bool mfa_required, bool user_has_totp, const std::string& mfa_token) {
  if (!mfa_required && !user_has_totp) {
    return {};
  }

  if (mfa_required && !user_has_totp) {
    return {true, json{{"must_setup_totp", true}, {"mfa_token", mfa_token}}};
  }

  return {true, json{{"mfa_required", true}, {"mfa_token", mfa_token}}};
}

std::string registration_eligibility_message(
  bool is_first_user,
...
password_auth::PasswordPolicy build_password_policy(
  ...
  policy.min_length = parse_int_setting_or(get_setting("password_min_length"), policy.min_length);

  policy.require_uppercase =
    parse_bool_setting_or(get_setting("password_require_uppercase"), policy.require_uppercase);
  policy.require_lowercase =
    parse_bool_setting_or(get_setting("password_require_lowercase"), policy.require_lowercase);
  policy.require_number =
    parse_bool_setting_or(get_setting("password_require_number"), policy.require_number);
  policy.require_special =
    parse_bool_setting_or(get_setting("password_require_special"), policy.require_special);
  policy.max_age_days =
    parse_int_setting_or(get_setting("password_max_age_days"), policy.max_age_days);
  policy.history_count =
    parse_int_setting_or(get_setting("password_history_count"), policy.history_count);
  return policy;
}
