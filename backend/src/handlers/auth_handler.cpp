...
template <bool SSL>
bool AuthHandler<SSL>::check_and_handle_mfa(uWS::HttpResponse<SSL>* res, const User& user,
...
template <bool SSL>
void AuthHandler<SSL>::handle_register_options(uWS::HttpResponse<SSL>* res, const std::string& body) {
    try {
...
template <bool SSL>
void AuthHandler<SSL>::handle_register_verify(uWS::HttpResponse<SSL>* res, const std::string& body) {
    try {
        if (!is_method_enabled("passkey")) {
            res->writeStatus("403")->writeHeader("Content-Type", "application/json")
                ->end(R"({"error":"Passkey authentication is not enabled on this server"})");
...
template <bool SSL>
void AuthHandler<SSL>::handle_add_device_passkey_options(uWS::HttpResponse<SSL>* res, const std::string& body) {
    try {
        if (!is_method_enabled("passkey")) {
            res->writeStatus("403")->writeHeader("Content-Type", "application/json")
...
template <bool SSL>
void AuthHandler<SSL>::handle_add_device_passkey_verify(uWS::HttpResponse<SSL>* res, const std::string& body) {
    try {
        auto j = json::parse(body);
        std::string credential_id = j.at("id");
        auto response = j.at("response");
...
template <bool SSL>
void AuthHandler<SSL>::handle_recovery_token_login(uWS::HttpResponse<SSL>* res, const std::string& body) {
    try {
        auto j = json::parse(body);
...
template <bool SSL>
void AuthHandler<SSL>::handle_request_access_options(uWS::HttpResponse<SSL>* res, const std::string& body) {
    try {
        auto mode = db.get_setting("registration_mode");
        std::string reg_mode = mode.value_or("invite");
...
template <bool SSL>
void AuthHandler<SSL>::handle_password_register(uWS::HttpResponse<SSL>* res, const std::string& body) {
    try {
        if (!is_method_enabled("password")) {
            res->writeStatus("403")->writeHeader("Content-Type", "application/json")
...
template <bool SSL>
void AuthHandler<SSL>::handle_password_change(uWS::HttpResponse<SSL>* res, const std::string& body,
...
template <bool SSL>
void AuthHandler<SSL>::handle_password_set(uWS::HttpResponse<SSL>* res, const std::string& body,
                                            const std::string& session_token) {
    try {
        auto user_id = db.validate_session(session_token);
        if (!user_id) {
...
template <bool SSL>
void AuthHandler<SSL>::handle_password_delete(uWS::HttpResponse<SSL>* res, const std::string& session_token) {
    try {
        auto user_id = db.validate_session(session_token);
        if (!user_id) {
            res->writeStatus("401")->writeHeader("Content-Type", "application/json")
...
template <bool SSL>
void AuthHandler<SSL>::handle_mfa_setup_verify(uWS::HttpResponse<SSL>* res, const std::string& body) {
    try {
        auto j = json::parse(body);
        std::string mfa_token = j.at("mfa_token");
...
template <bool SSL>
void AuthHandler<SSL>::handle_request_status(uWS::HttpResponse<SSL>* res, const std::string& request_id) {
    try {
