...
struct CborReader {
    ...
    void ensure(size_t n) const {
        if (pos + n > size)
            throw std::runtime_error("CBOR: unexpected end of data at pos " +
                                     std::to_string(pos) + " need " + std::to_string(n));
    }

    uint8_t read_u8() { ensure(1); return data[pos++]; }

    // Read the "additional info" value (may consume 0, 1, 2, 4, or 8 extra bytes)
    uint64_t read_additional(uint8_t additional) {
        if (additional < 24) return additional;
        if (additional == 24) { return read_u8(); }
        if (additional == 25) {
            ensure(2);
    ...
    std::vector<unsigned char> read_bstr() {
        ...
        uint8_t major = b >> 5;
        uint8_t additional = b & 0x1f;
        if (major != 2) throw std::runtime_error("CBOR: expected byte string, got major type " + std::to_string(major));
    ...
    size_t read_map_header() {
        ...
        uint8_t major = b >> 5;
        uint8_t additional = b & 0x1f;
        if (major != 5) throw std::runtime_error("CBOR: expected map, got major type " + std::to_string(major));
    ...
    void skip() {
        ...
        uint64_t val = read_additional(additional);
        switch (major) {
        case 0: case 1: break; // integers already fully read
        case 2: case 3: ensure(val); pos += val; break; // byte/text string
        case 4: for (uint64_t i = 0; i < val; i++) skip(); break; // array
        case 5: for (uint64_t i = 0; i < val * 2; i++) skip(); break; // map
        case 7: break; // simple/float
        default: throw std::runtime_error("CBOR: cannot skip major type " + std::to_string(major));
        }
    }
};
...
static CoseKeyData parse_cose_ec_key(const std::vector<unsigned char>& cose_bytes) {
    ...
    for (size_t i = 0; i < map_len; i++) {
        ...
        switch (label) {
        case 1:  key.kty = static_cast<int>(reader.read_int()); break; // kty
        case 3:  key.alg = static_cast<int>(reader.read_int()); break; // alg
        case -1: key.crv = static_cast<int>(reader.read_int()); break; // crv
        case -2: key.x = reader.read_bstr(); break;                    // x
        case -3: key.y = reader.read_bstr(); break;                    // y
        default: reader.skip(); break; // skip unknown labels
        }
    }
    return key;
}
...
static void parse_auth_data(const std::vector<unsigned char>& auth_data, AuthDataParsed& out) {
    ...
    out.sign_count = (static_cast<uint32_t>(auth_data[33]) << 24) |
                     (static_cast<uint32_t>(auth_data[34]) << 16) |
                     (static_cast<uint32_t>(auth_data[35]) << 8) |
                     static_cast<uint32_t>(auth_data[36]);
    ...
    if (out.flags & 0x40) {
        ...
        uint16_t cred_id_len = (static_cast<uint16_t>(auth_data[53]) << 8) |
                                static_cast<uint16_t>(auth_data[54]);
        ...
        out.credential_id_b64url = base64url_encode(
            auth_data.data() + cred_id_start, cred_id_len);
        ...
        if (cose.kty != 2 || cose.alg != -7) {
            throw std::runtime_error("Unsupported COSE key type: kty=" + std::to_string(cose.kty) +
        ...
        if (cose.x.size() != 32 || cose.y.size() != 32) {
            throw std::runtime_error("Invalid EC key coordinate size: x=" + std::to_string(cose.x.size()) +
...
static bool verify_es256(const std::vector<unsigned char>& public_key_xy,
...
static bool verify_client_data(const std::string& client_data_json_str,
    ...
    try {
        ...
        if (origin != expected_origin) {
            ...
            if (!both_localhost) {
                std::cerr << "[WebAuthn] Origin mismatch: got '" << origin
                          << "', expected '" << expected_origin << "'" << std::endl;
                return false;
            }
        }
...
std::optional<ParsedCredential> verify_registration(
    ...
    if (!verify_client_data(client_data_str, "webauthn.create", expected_challenge, expected_origin)) {
...
bool verify_pki_signature(const std::string& public_key_spki_b64url,
    ...
    if (EVP_DigestVerifyInit(md_ctx, nullptr, EVP_sha256(), nullptr, pkey) == 1) {
        ...
        if (rc == 1) {
            ...
            if (der_len > 0 && der_sig) {
                ...
                if (EVP_DigestVerifyInit(md_ctx, nullptr, EVP_sha256(), nullptr, pkey) == 1) {
                    rc = EVP_DigestVerify(md_ctx, der_sig, der_len,
                                           challenge_bytes, challenge_len);
                    valid = (rc == 1);
                }
