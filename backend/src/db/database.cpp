...
// DEPRECATED in P1.1 PR-B (sqitch cutover): schema is now managed
// externally via sqitch (see sqitch/README.md). Kept as a callable no-op
// so existing call sites (main.cpp, integration tests) don't need surgery.
void Database::run_migrations() {
  LOG_WARN_N(
    "db",
    nullptr,
    "run_migrations() is a deprecated no-op; schema is managed by sqitch. "
    "See sqitch/README.md.");
}
