...
export function DeviceManager() {
  ...
  const handleRemoveDevice = async (id: string) => {
    if (!confirm('Remove this device? It will no longer be able to sign in.')) return;
    setError('');
    try {
        ...
        {devices.map((device) => (
          <Card key={device.id}>
            <CardBody className="flex-row items-center justify-between py-2">
              <div>
                <div className="text-sm text-foreground">{device.device_name}</div>
                <div className="text-xs text-default-500">
                  Added {new Date(device.created_at).toLocaleDateString()}
                </div>
              </div>
              {devices.length > 1 && (
                <Button color="danger" variant="light" size="sm" onPress={() => handleRemoveDevice(device.id)}>
                  Remove
                </Button>
              )}
            </CardBody>
        ...
        <Card className="border-primary/30 bg-primary/10">
          <CardBody className="space-y-3">
            <p className="text-sm text-primary-300">
              Scan this QR code with your new device, or enter the code manually (expires in 15 minutes):
            </p>
            ...
            <p className="text-xs text-default-500">
              On the new device, go to the login page and click "Link existing account to this device".
            </p>
          </CardBody>
        </Card>
      ) : (
