...
export function Header({ onShowAdmin, onShowSettings, onToggleSidebar, onShowChannelSettings }: Props) {
  ...
  const canManageChannel = activeChannel && !activeChannel.is_direct &&
  ...
  return (
    <header className="bg-content1 border-b border-default-100 px-3 sm:px-4 py-2 flex items-center justify-between">
      ...
      <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
        ...
        <Button variant="light" size="sm" color="default" onPress={handleLogout}>
          Logout
        </Button>
      </div>
    </header>
