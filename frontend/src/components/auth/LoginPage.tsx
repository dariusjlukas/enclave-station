...
import { Button, Card, CardBody, Select, SelectItem, Alert } from '@heroui/react';
import { useChatStore } from '../../stores/chatStore';
import { getStoredKeys, signChallenge, listStoredUsers } from '../../services/crypto';
...
export function LoginPage({ onSwitchToRegister, onSwitchToRequest, onSwitchToAddDevice }: Props) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const result = await api.verifyChallenge(keys.publicKeyPem, challenge, signature);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-background">
      ...
      <Card className="w-full max-w-md mx-4 sm:mx-auto shadow-2xl">
        <CardBody className="p-5 sm:p-8">
          ...
          <p className="text-default-500 mb-6">Sign in with your cryptographic key</p>
          ...
          {storedUsers.length === 0 ? (
            <div className="text-default-500 text-center py-4 mb-4">
              No keys found on this device. Register a new account or link an existing one.
            </div>
          ...
          <div className="mt-6 flex flex-col gap-2 text-center">
            <Button variant="light" color="primary" fullWidth onPress={onSwitchToRegister} size="sm">
              Have an invite? Register here
            </Button>
            <Button variant="light" color="primary" fullWidth onPress={onSwitchToAddDevice} size="sm">
              Link existing account to this device
            </Button>
            <Button variant="light" color="default" fullWidth onPress={onSwitchToRequest} size="sm">
              Request access
            </Button>
          </div>
        </CardBody>
